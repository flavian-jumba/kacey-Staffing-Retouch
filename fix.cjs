const fs = require('fs');
let code = fs.readFileSync('src/pages/Application.tsx', 'utf8');

const bgString = `      {/* ── Mesh Gradient Background ── */}
...
// We replace everything between `{/* ── Mesh Gradient Background ── */}` and `<div className="relative z-10 w-full h-full">`
const regex = /\{\/\* ── Mesh Gradient Background ── \*\/\}(.|\n)*?(?=<div className="relative z-10 w-full h-full">)/;
code = code.replace(regex, bgString + '\n\n      ');
fs.writeFileSync('src/pages/Application.tsx', code);
